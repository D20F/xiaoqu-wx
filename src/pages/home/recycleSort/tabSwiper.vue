<template>
    ...
    >
        ...
        >
            <swiper-item class="u-swiper-item" v-for="(item, index) in tabList" :key="index">
                <view class="content">
                    <view
                        class="content_row"
                        :class="{green:selected == i.name}"
                        v-for="(i,indexs) in item"
                        :key="indexs"
                        @click="selectedFun(i)"
                    >
                        <image v-if="selected == i.name" :src="i.srcs" />
                        <image v-else :src="i.src" />
                        <text :class="{font_white:selected == i.name}">{{i.title}}</text>
                    </view>
                </view>
            </swiper-item>
        </swiper>
        ...
        >
            ...
            <block v-if="mode == 'number'">
                <view class="u-indicator-item-number">{{ current + 1 }}/{{ tabList.length }}</view>
            </block>
        </view>
    </view>
</template>
 ...
 * @property {Array} tabList 轮播图数据，见官网"基本使用"说明
 ...
 * @example <u-swiper :tabList="tabList" mode="dot" indicator-pos="bottomRight"></u-swiper>
 */
export default {
    name: "u-swiper",
    props: {
    ...
    computed: {
        justifyContent() {
            if (
                this.indicatorPos == "topLeft" ||
                this.indicatorPos == "bottomLeft"
            )
                return "flex-start";
            if (
    ...
    methods: {
        ...
        selectedFun(item) {
            this.$emit("selectedFun", item.name);
        },
    ...
    created() {
        uni.$on("currentClick", this.currentClick);
    },
    beforeDestroy() {
        uni.$off("currentClick", this.currentClick);
    },
};
</script>
